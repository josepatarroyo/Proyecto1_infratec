...
void insertarMensaje( Imagen * img , unsigned char mensaje[], int n ) {
	...
	unsigned char fin = mensaje[0];
	unsigned char agregador ;
	...
	while ((tamanio*8) > pos)
	{
		...
		agregador = sacarNbits(mensaje, pos, n);

		
		img->informacion[rgbIm] >> n;
		img->informacion[rgbIm] << n;
		img->informacion[rgbIm] = img->informacion[rgbIm] & agregador;
		pos += n;
	}
}
...
unsigned char sacarNbits( unsigned char secuencia[], int bitpos, int n ) {
	...
	retorno = secuencia[num];

	retorno << res;
	retorno >> res;

	int condicion = (8 - res) - n;
